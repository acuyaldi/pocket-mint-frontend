...
function formatDueDate(value: string, intlLocale: string) {
...
export function BillCard({ bill, onPay }: { bill: BillDto; onPay: (bill: BillDto) => void }) {
  const t = useTranslations("tagihan.card");
  ...
  return (
    <article className={`flex flex-col rounded-xl border border-border/70 border-t-4 bg-card p-5 shadow-sm ${isOverdue ? "border-t-coral" : "border-t-mint"}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {bill.walletName} · {isFull ? t("oneTime") : t("installment")}
          </p>
        </div>
        <span className={`rounded-full px-2 py-1 text-[10px] font-bold uppercase tracking-wide ${isOverdue ? "bg-coral/10 text-coral" : "bg-mint/10 text-mint"}`}>
          {isOverdue ? t("overdue") : bill.status === "SETTLED" ? t("settled") : t("active")}
        </span>
      </div>

      <div className="mt-5 space-y-3">
        <div className="flex items-baseline justify-between gap-4">
          <span className="text-xs text-muted-foreground">{t("termAmount")}</span>
          <strong className="text-lg tabular-nums text-foreground">{formatCurrency(bill.amountPerTerm, intlLocale)}</strong>
        </div>
        <div className="flex items-center justify-between gap-4">
          <span className="flex items-center gap-1.5 text-xs text-muted-foreground">
            <CalendarClock className="size-4" /> {t("dueDate")}
          </span>
          <span className={isOverdue ? "text-sm font-semibold text-coral" : "text-sm font-medium text-foreground"}>
            {formatDueDate(bill.nextDueDate, intlLocale)}
          </span>
        </div>
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          <span>{t("termsPaid", { paid: bill.paidTerms, total: bill.totalTerms })}</span>
          <span>{t("remaining", { amount: formatCurrency(remaining, intlLocale) })}</span>
        </div>
      ...
      {isPayable ? (
        ...
        >
          <Banknote className="size-4" /> {t("pay")}
        </button>
      ) : null}
    </article>
  );
