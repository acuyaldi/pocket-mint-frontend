...
export default function CicilanPage() {
  ...
  const status = useMemo(() => {
    ...
    if (overdue.length > 0) return { text: `${overdue.length} Terlambat`, color: "#EF4444" };
    if (upcoming.length > 0) return { text: `${upcoming.length} Upcoming`, color: "#F59E0B" };
    return { text: "Semua On-Track", color: "#10B981" };
  }, [all]);
