...
This document is an agent-facing derivative of the authoritative Pocket Mint `DESIGN.md` (`../pocket-mint-docs/docs/product/stictch/core/DESIGN.md`). If any wording differs, `DESIGN.md` wins. Do not treat existing implementation as permission to override the authority.
...
- Use the light off-white canvas and white card surfaces defined in `app/globals.css`.
- Use Slate for navigation, primary text emphasis, and solid primary buttons.
- Use Mint only for assets, income, positive values, and focus indication.
...
Desktop is the master breakpoint. Use a maximum content width of 1280px, an 8px spacing rhythm, 32px major section spacing, and the safe areas defined in `DESIGN.md`.
...
For implementation-level token values, component states, and cross-references to Storybook/routing/auth/API, see `ui-system.skill.md` and `frontend-architecture.skill.md` — this file does not duplicate them.
