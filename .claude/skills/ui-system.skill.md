...
> Read this before touching any component, page, or style file. The authoritative source is `../pocket-mint-docs/docs/product/stictch/core/DESIGN.md`; `design.md` is its frontend-agent summary. When code or older guidance conflicts, the authoritative `DESIGN.md` wins.
...
Source of truth: `app/globals.css` `@theme inline`. These are the ONLY valid color values:

| Semantic role | Established value | Use |
|---|---:|---|
| Background / Canvas | `#f9f9f8` | Application background |
| Card / Surface | `#ffffff` | Cards, panels, dialogs, inputs |
| Muted surface | `#f4f3f3` | Restrained secondary surfaces and hover states |
| Surface high | `#eeeeed` | Elevated secondary surfaces |
| Border | `#c0c8c7` | Structural borders and dividers |
| Foreground / Ink | `#1a1c1c` | Primary text and financial values |
| Muted foreground | `#414848` | Supporting text and metadata |
| Mint | `#2dd4bf` | Assets, income, positive values, focus ring |
| Slate | `#0f172a` | Navigation, neutral emphasis, solid primary buttons |
| Amber / Warning | `#f59e0b` | Warning, upcoming, pending, attention |
| Coral | `#f87171` | Debt/destructive decorative fills, borders, badges |
| Coral (AA-safe) | `#c1272d` | `--color-destructive` / `--color-coral-strong` — use for text on light surfaces (form errors, error banners) that must hit a 4.5:1 contrast ratio |
| Accent | `#d3e4fe` | Restrained secondary accent |

Mint is not the default primary-button fill. Primary buttons are solid Slate. Ordinary expenses use Ink, not Coral. Never communicate state through color alone. When rendering Coral as body text, prefer the AA-safe `--color-destructive` value over the brighter decorative `--color-coral`.

## Typography and Financial Values

- Use Inter for display, headings, body, labels, metadata, and financial figures (`--font-sans`).
...
- Preserve full, locale-formatted values via `formatCurrency` (`lib/utils.ts`) — never write a new formatter. Long amounts may resize or wrap as one unit but must not be misleadingly rounded or truncated.
...
- Default radius: `--radius-md` (12px equivalent, `0.75rem`).
- Large-container radius: `--radius-lg` (16px equivalent, `1rem`).
- Pill radius: `--radius-full` (`9999px`) only for true pills and status labels.
...
## Accessibility Expectations (verified)

- Minimum 44px touch targets on interactive controls.
- Visible, high-contrast focus indicators; never remove `:focus-visible` styling.
- Status is never color-only — pair with text, icon, or `aria-*` state.
- Every interactive element has an accessible name (label, `aria-label`, or visible text).
- Storybook's `a11y` addon runs with `test: "error"` (`.storybook/preview.tsx`) — accessibility violations fail the Storybook Vitest project, which is a CI gate. See `frontend-architecture.skill.md` → Storybook.

This documents verified repository expectations, not a WCAG-level compliance claim.
...
Storybook configuration, routing/auth, the API layer, and TanStack Query are owned by `frontend-architecture.skill.md` — this file only covers visual/component conventions and cross-references them where relevant.
